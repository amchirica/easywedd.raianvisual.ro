...
export async function listContractsDirectory(options?: {
  ...
  const workspaceIds = [...new Set(contracts.map((c) => c.workspace_id))];
  const userIds = [
    ...new Set(contracts.map((c) => c.user_id).filter(Boolean) as string[]),
  ];

  const [{ data: workspaces }, { data: profiles }] = await Promise.all([
    admin.from("workspaces").select("id, name, owner_id").in("id", workspaceIds),
    userIds.length
  ...
  return contracts.map((c) => {
    ...
    const email =
      ...
      (ws ? emailById.get(ws.owner_id) : null) ??
      "—";
    return {
      id: c.id,
      title: c.title,
