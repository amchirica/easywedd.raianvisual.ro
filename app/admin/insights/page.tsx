...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title: dict.admin.insightsMetaTitle };
}
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="font-heading text-4xl">{dict.admin.insightsTitle}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            Agregări anonimizate · min 20 workspace-uri · doar cu consimțământ
          </p>
        </div>
        <form action={refreshIndustryInsightsAction}>
          <Button type="submit">{dict.admin.runAggregation}</Button>
        </form>
      </header>
